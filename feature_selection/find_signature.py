...
print "Accuracy on test: %.4f" % acc
print "Accuracy on train: %.4f" % accuracy_score(clf.predict(features_train), labels_train)

most_important = []

for entry in range(len(clf.feature_importances_)):
    importance = clf.feature_importances_[entry]
    if importance > 0.05:
        print entry," : ",importance
        most_important.append(entry)
